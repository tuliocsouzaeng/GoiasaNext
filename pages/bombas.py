...
bombas = load_csv("data/bombas.csv.enc", sep=";", encoding="utf-8")
#bombas = pd.read_csv("data/bombas.csv", sep=";", encoding='utf-8')
...
with st.expander("Abrir Filtros"):
    setor_selecionado = st.multiselect("Selecione o Setor", bombas["SETOR"].dropna().unique().tolist())
    criticidade_selecionada = st.multiselect("Selecione a Criticidade", bombas["CRITICIDADE"].dropna().unique().tolist())
    tipo_bomba_selecinado = st.multiselect("Selecione o tipo de bomba", bombas["TIPO DE BOMBA"].dropna().unique().tolist())
    modelo_selecionado = st.multiselect("Selecione o Modelo", bombas["TIPO/MODELO"].dropna().unique().tolist())
    equipamento_selecionado = st.multiselect("Selecione o Equipamento", bombas["IDENTIFICAÇÃO"].dropna().unique().tolist())
    TAG_selecionada = st.multiselect("Selecione a TAG", bombas["TAG"].dropna().unique().tolist())
    anofab_selecionado = st.multiselect("Selecione o Ano de Fabricação", bombas["ANO FABR."].dropna().unique().tolist())


# Aplicar os filtros ao DataFrame
bombas_filtradas = bombas.copy()

if setor_selecionado:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["SETOR"].isin(setor_selecionado)]
if criticidade_selecionada:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["CRITICIDADE"].isin(criticidade_selecionada)]
if modelo_selecionado:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["TIPO/MODELO"].isin(modelo_selecionado)]
if equipamento_selecionado:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["IDENTIFICAÇÃO"].isin(equipamento_selecionado)]
if TAG_selecionada:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["TAG"].isin(TAG_selecionada)]
if tipo_bomba_selecinado:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["TIPO DE BOMBA"].isin(tipo_bomba_selecinado)]
if anofab_selecionado:
    bombas_filtradas = bombas_filtradas[bombas_filtradas["ANO FABR."].isin(anofab_selecionado)]

# _______________ Geração de Informações ________________________

# Calcular o Total de Motores
total_bombas = bombas.shape[0]

# Calcular o Total de Motores Filtrados
total_bombas_filtradas = bombas_filtradas.shape[0]

# Contar a quantidade de equipamentos por setor
bombas_filtradas['SETOR'] = bombas_filtradas['SETOR'].fillna('Desconhecido')
setor_count = bombas_filtradas['SETOR'].value_counts().reset_index()
setor_count.columns = ['SETOR', 'Quantidade']

# Contar a quantidade de equipamentos por criticidade
bombas_filtradas['CRITICIDADE'] = bombas_filtradas['CRITICIDADE'].fillna('Desconhecido')
criticidade_count = bombas_filtradas['CRITICIDADE'].value_counts().reset_index()
criticidade_count.columns = ['CRITICIDADE', 'Quantidade']

# Contar a quantidade de equipamentos por fornecedor
bombas_filtradas['FORNECEDOR'] = bombas_filtradas['FORNECEDOR'].fillna('Desconhecido')
fornecedor_count = bombas_filtradas['FORNECEDOR'].value_counts().reset_index()
...
anofab_count = bombas_filtradas['ANO FABR.'].value_counts(dropna=False).reset_index()
...
with l1c1:
    ...
    st.metric("Total de Equipamentos", total_bombas)

with l1c2:
    # Criar o gráfico de donut
    fig_donut = px.pie(
        names=["Selecionados", "Não Selecionados"],
        values=[total_bombas_filtradas, total_bombas - total_bombas_filtradas],
...
with l2c1:
    if not bombas_filtradas.empty and 'SETOR' in bombas_filtradas.columns:
...
with l3c1:
    if not bombas_filtradas.empty and 'CRITICIDADE' in bombas_filtradas.columns:
...
with l4c1:
    if not bombas_filtradas.empty and 'FORNECEDOR' in bombas_filtradas.columns:
        # Gráfico de barras para Criticidade
...
with l5c1:
    if not bombas_filtradas.empty and 'ANO FABR.' in bombas_filtradas.columns:
        # Gráfico de barras para Setor
...
with l6c1:
    with st.expander("Mais Detalhes"):
        ...
        st.dataframe(bombas_filtradas, height=400)